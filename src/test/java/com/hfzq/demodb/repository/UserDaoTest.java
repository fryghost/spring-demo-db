...
import com.hfzq.demodb.DemoDbApplicationTests;
 ...
 **/
@Slf4j
public class UserDaoTest extends DemoDbApplicationTests {
    ...
    public void testSave() {
        ...
        User testUser = User.builder().name("testSave4").password(SecureUtil.md5("123456" + salt)).salt(salt).email("testSave4@email.com").phoneNumber("17300000004").status(1).lastLoginTime(new DateTime()).build();

        userDao.save(testUser);

        Assert.assertNotNull(testUser.getId());
        Optional<User> byId = userDao.findById(testUser.getId());
